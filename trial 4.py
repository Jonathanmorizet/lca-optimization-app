    ...
    if analysis_mode == "Multi-Farm Comparison & Hybrid" and len(farms_data) > 1:
        ...
        if st.button("🚀 Generate Hybrid Strategy"):
            with st.spinner("Creating hybrid strategy..."):
                ...
                hybrid_costs = []
                hybrid_gwp_values = []
                hybrid_sources = []  # Track which farm each material comes from
                ...
                for material in unique_materials:
                    ...
                    if candidates:
                        ...
                        hybrid_sources.append(best['farm'])  # Track source
                ...
                # Show detailed hybrid implementation plan
                st.markdown("---")
                st.markdown("### 📋 Hybrid Strategy Implementation Plan")
                
                # Create detailed breakdown showing which farm each material comes from
                ...
                # Sort by cost to prioritize high-impact changes
                hybrid_df = hybrid_df.sort_values('Total Cost ($)', ascending=False)
                ...
                # Extract year information from merged_df if available
                st.markdown("### 📅 Timeline: When to Implement Each Material")
                
                # Try to get year info from the original dataframes
                timeline_data = []
                for i, material in enumerate(hybrid_materials):
                    source_farm = next(farm for farm in farms_data if farm['name'] == hybrid_sources[i])
                    
                    # Check if Year column exists in the merged_df
                    if 'Year' in source_farm['merged_df'].columns:
                        mat_idx = source_farm['materials'].index(material)
                        # Get year from the dataframe - need to map back to original row
                        year_matches = source_farm['merged_df'][source_farm['merged_df']['Material'] == material]
                        if not year_matches.empty:
                            year = year_matches.iloc[0]['Year'] if 'Year' in year_matches.columns else 'N/A'
                        else:
                            year = 'N/A'
                    else:
                        year = 'N/A'
                    ...
                    timeline_data.append({
                        'Material': material,
                        'Year': year,
                        'Source': hybrid_sources[i],
                        'Amount': hybrid_amounts[i],
                        'Cost': hybrid_amounts[i] * hybrid_costs[i],
                        'GWP': hybrid_gwp_values[i]
                    })
                
                timeline_df = pd.DataFrame(timeline_data)
                
                # If we have year data, organize by year
                if timeline_df['Year'].notna().any() and (timeline_df['Year'] != 'N/A').any():
                    timeline_df_sorted = timeline_df.sort_values(['Year', 'Cost'], ascending=[True, False])
                    
                    st.markdown("**Implementation Schedule by Year:**")
                    
                    for year in sorted(timeline_df_sorted['Year'].unique()):
                        if year != 'N/A':
                            year_data = timeline_df_sorted[timeline_df_sorted['Year'] == year]
                            
                            with st.expander(f"📆 Year {year} - {len(year_data)} materials (${year_data['Cost'].sum():.2f}, {year_data['GWP'].sum():.2f} kg CO2-Eq)"):
                                st.dataframe(year_data[['Material', 'Source', 'Amount', 'Cost', 'GWP']], use_container_width=True)
                                
                                # Show which strategy to follow this year
                                dominant_source = year_data['Source'].mode()[0] if not year_data['Source'].mode().empty else year_data['Source'].iloc[0]
                                st.info(f"💡 **Primary Strategy for Year {year}:** Follow **{dominant_source}** approach")
                else:
                    st.warning("⚠️ No year information found in the uploaded files. Showing materials without timeline.")
                    st.dataframe(timeline_df[['Material', 'Source', 'Amount', 'Cost', 'GWP']], use_container_width=True)
                
                # Actionable recommendations
                st.markdown("---")
                st.markdown("### 💡 Actionable Recommendations")
                
                # Find materials where there's significant difference between farms
                recommendations = []
                for material in unique_materials:
                    costs_for_material = []
                    gwps_for_material = []
                    ...
                    for farm in farms_data:
                        if material in farm['materials']:
                            idx = farm['materials'].index(material)
                            costs_for_material.append(farm['base_amounts'][idx] * farm['costs'][idx])
                            
                            if "kg CO2-Eq/Unit" in farm['impact_columns']:
                                gwp_idx = farm['impact_columns'].index("kg CO2-Eq/Unit")
                                gwps_for_material.append(farm['base_amounts'][idx] * farm['impact_matrix'][idx, gwp_idx])
                    
                    if len(costs_for_material) > 1:
                        cost_diff = (max(costs_for_material) - min(costs_for_material)) / max(costs_for_material) * 100
                        if cost_diff > 10:  # More than 10% difference
                            selected_source = hybrid_sources[hybrid_materials.index(material)]
                            recommendations.append(f"🎯 **{material}**: Use **{selected_source}** approach - saves {cost_diff:.1f}% on cost compared to alternatives")
                
                if recommendations:
                    st.markdown("**Key Implementation Priorities:**")
                    for rec in recommendations[:5]:  # Show top 5
                        st.markdown(rec)
                else:
                    st.info("All farms use similar approaches for common materials.")
