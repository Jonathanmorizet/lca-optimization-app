...
# This is a high-level skeleton Python app using Streamlit to create an interactive optimization dashboard.
...
# Replace with your merged_df from NSGA-II script
@st.cache_data
def load_data():
    # Dummy placeholder structure (replace with real data load)
    df = pd.read_excel("DEAP NSGA Readable file.xlsm", sheet_name=None)
    inputs_df = df[list(df.keys())[0]]
    cost_df = df[list(df.keys())[1]]
    impact_df = df[list(df.keys())[2]]
    return inputs_df, cost_df, impact_df

inputs_df, cost_df, impact_df = load_data()
...
# Material inputs and impact categories
material_names = merged_df["Material"].tolist()
impact_cols = [col for col in impact_df.columns if col not in ["Material", "Unit"]]
...
scenario = st.sidebar.selectbox(
    "Select Scenario",
    [
        "Optimize All Impacts (Tradeoff)",
        "Optimize Single Impact",
        "Optimize Cost + Selected Impact",
        "Optimize Cost Only"
    ]
)

# Global deviation slider
global_dev = st.sidebar.slider("Global +/- % Deviation", 0, 100, 20)

# Toggle to enable per-material input bounds
use_custom_bounds = st.sidebar.checkbox("Set individual +/- % deviations")

# Bounds container
bounds = {}
for material in material_names:
    if use_custom_bounds:
        deviation = st.sidebar.slider(f"{material} +/- %", 0, 100, global_dev)
    else:
        deviation = global_dev
    bounds[material] = deviation

# Select impact category if needed
selected_impact = None
if scenario in ["Optimize Single Impact", "Optimize Cost + Selected Impact"]:
    selected_impact = st.selectbox("Select TRACI Impact Category", impact_cols)

# Dummy button to trigger optimization (replace this block with real DEAP optimization)
if st.button("Run Optimization"):
    st.write(f"**Running scenario**: {scenario}")

    # Placeholder result generation (replace with NSGA-II or DEAP results)
    np.random.seed(0)
    optimized_inputs = np.random.rand(len(material_names)) * 100
    costs = np.random.rand(len(material_names)) * 10
    impacts = {col: np.random.rand() * 100 for col in impact_cols}

    # Display summary
    st.subheader("Optimization Summary")
    st.metric("Total Cost", f"${np.dot(optimized_inputs, costs):.2f}")

    if selected_impact:
        st.metric(f"{selected_impact} Impact", f"{impacts[selected_impact]:.2f}")

    st.subheader("Top 5 Most Sensitive Inputs")
    sorted_indices = np.argsort(costs)[-5:][::-1]
    top_inputs = [(material_names[i], optimized_inputs[i], bounds[material_names[i]]) for i in sorted_indices]
    for name, value, dev in top_inputs:
        st.write(f"**{name}** - Optimized Amount: {value:.2f}, Bound +/- {dev}%")

    st.subheader("Impact Overview")
    st.bar_chart(pd.DataFrame.from_dict(impacts, orient='index', columns=['Impact Value']))
...
!streamlit run /content/your_streamlit_app.py

# Save the content of cell NbPcvRxvzKA4 to a Python file
...
# Wait for the app to start and get the external URL
time.sleep(10) # Give the app some time to start
logs = open('/content/logs.txt', 'r').read()
url = re.search("External URL: (.*?)\n", logs).group(1)

print(f"Click the link to access your Streamlit app: {url}")
...
# This ensures deap is available in the Streamlit environment
try:
    from deap import base, creator, tools, algorithms
except ImportError:
    import subprocess
...
def load_data():
    # Assuming the Excel file is uploaded and named 'DEAP NSGA Readable file.xlsm'
    # In a deployed Streamlit app, you might need a different way to handle file uploads
    # or include the file with your app. For Colab environment, this works with uploaded file.
    try:
        df = pd.read_excel("DEAP NSGA Readable file.xlsm", sheet_name=None)
        inputs_df = df[list(df.keys())[0]]
        cost_df = df[list(df.keys())[1]]
        ...
        merged_df = inputs_df.merge(cost_df, on=["Material", "Unit"], how="left")
        merged_df = merged_df.merge(impact_df, on=["Material", "Unit"], how="left")
        merged_df = merged_df.fillna(0) # Fill missing values with 0 as done previously
        ...
        impact_matrix = merged_df[impact_columns].values
        traci_impact_cols = impact_columns # Use the aligned impact columns

        return merged_df, materials, base_amounts, costs, impact_matrix, traci_impact_cols

    except FileNotFoundError:
        st.error("Error: 'DEAP NSGA Readable file.xlsm' not found. Please upload the file.")
        return None, None, None, None, None, None

# --- DEAP Optimization Functions (Placeholders/Adaptations) ---

# Need to define evaluate functions, toolbox, creator classes here or import them
# from separate files if your app grows.

# Example adaptation for a multi-objective evaluation function (Cost + GWP)
def evaluate_cost_gwp(individual, costs, impact_matrix, traci_impact_cols):
    ...
    try:
        ...
        st.warning("GWP column not found in data.")

    return total_cost, total_gwp

# Example adaptation for a single-objective evaluation function (Cost + Combined Impact)
def evaluate_cost_combined(individual, costs, impact_matrix):
     ...
     return (total_cost + combined_traci_impact_value,) # Return as tuple

# Placeholder for running NSGA-II (adapt from your previous code)
def run_nsga2_optimization(population_size, generations, cxpb, mutpb, low_bounds, high_bounds, costs, impact_matrix, traci_impact_cols):
    # Attempt to delete previously created classes to avoid RuntimeWarning
    try:
        del creator.FitnessMin
    except AttributeError:
    ...
    # Create Fitness class with two objectives to minimize (Cost and GWP for this example)
    creator.create("FitnessMin", base.Fitness, weights=(-1.0, -1.0))
    creator.create("Individual", list, fitness=creator.FitnessMin)

    toolbox = base.Toolbox()
    toolbox.register("individual", tools.initIterate, creator.Individual,
                     lambda: [random.uniform(low, high) for low, high in zip(low_bounds, high_bounds)])
    toolbox.register("population", tools.initRepeat, list, toolbox.individual)

    # Register the evaluation function for Cost vs GWP
    ...
    algorithms.eaMuPlusLambda(pop, toolbox, mu=population_size, lambda_=population_size,
                              cxpb=cxpb, mutpb=mutpb, ngen=generations, verbose=False)

    return tools.sortNondominated(pop, k=len(pop), first_front_only=True)[0]

# Placeholder for running single-objective optimization (adapt from your previous code)
def run_single_objective_optimization(objective_func, population_size, generations, cxpb, mutpb, low_bounds, high_bounds, *args):
     # Attempt to delete previously created classes to avoid RuntimeWarning
     try:
         del creator.FitnessMin
     except AttributeError:
     ...
     toolbox.register("evaluate", objective_func, *args) # Pass additional arguments
...
# Load data
merged_df, materials, base_amounts, costs, impact_matrix, traci_impact_cols = load_data()
...
if merged_df is not None:
    ...
    scenario = st.sidebar.selectbox(
        "Select Scenario",
        [
            "Optimize Cost vs GWP (Tradeoff)", # Example Multi-objective
            "Optimize Cost + Combined Impact", # Example Single-objective
            "Optimize Single Impact",
            "Optimize Cost Only"
        ]
    )
    ...
    # Bounds container
    bounds = {}
    low_bounds = np.copy(base_amounts)
    high_bounds = np.copy(base_amounts)

    st.sidebar.subheader("Material Input Bounds (+/- %)")
    if use_custom_bounds:
        for i, material in enumerate(materials):
            deviation = st.sidebar.number_input(f"{material}", min_value=0, max_value=100, value=global_dev, key=f"dev_{material}")
            bounds[material] = deviation
            low_bounds[i] = max(0, base_amounts[i] * (1 - deviation / 100.0))
            high_bounds[i] = base_amounts[i] * (1 + deviation / 100.0)
    else:
        deviation = global_dev
        bounds = {mat: deviation for mat in materials}
        low_bounds = np.maximum(0, base_amounts * (1 - deviation / 100.0))
        high_bounds = base_amounts * (1 + deviation / 100.0)
    ...
    NGEN = st.sidebar.slider("Number of Generations", 10, 200, 40)
    ...
    selected_impact = None
    if scenario in ["Optimize Single Impact", "Optimize Cost + Selected Impact"]:
        selected_impact = st.selectbox("Select TRACI Impact Category for Single Optimization", traci_impact_cols)


    if st.button("Run Optimization"):
        st.subheader(f"Running Scenario: {scenario}")

        if scenario == "Optimize Cost vs GWP (Tradeoff)":
            st.write("Finding Pareto front for minimizing Total Cost and Global Warming Potential (GWP).")
            pareto_front = run_nsga2_optimization(POP_SIZE, NGEN, CXPB, MUTPB, low_bounds, high_bounds, costs, impact_matrix, traci_impact_cols)

            st.subheader("Pareto Front Solutions (Cost vs. GWP)")
            if pareto_front:
                pareto_data = []
                for ind in sorted(pareto_front, key=lambda x: x.fitness.values[0]):
                     pareto_data.append({
                         'Total Cost ($)': ind.fitness.values[0],
                         'Total GWP (kg CO2-Eq)': ind.fitness.values[1]
                     })
                ...
                fig, ax = plt.subplots()
                ax.scatter(pareto_df['Total Cost ($)'], pareto_df['Total GWP (kg CO2-Eq)'])
                ax.set_xlabel('Total Cost ($)')
                ax.set_ylabel('Total GWP (kg CO2-Eq)')
                ax.set_title('Pareto Front: Total Cost vs. GWP')
                st.pyplot(fig)


            else:
                st.write("No non-dominated solutions found.")

        elif scenario == "Optimize Cost + Combined Impact":
             st.write("Finding the optimal solution minimizing Total Cost + Sum of all TRACI Impacts.")
             best_individual_combined = run_single_objective_optimization(
                 evaluate_cost_combined, POP_SIZE, NGEN, CXPB, MUTPB, low_bounds, high_bounds, costs, impact_matrix
             )

             st.subheader("Optimal Solution for Total Cost + Combined TRACI Impact")
             if best_individual_combined:
                 # Evaluate this individual using the function that returns all objectives for display
                 # Need a function that evaluates all objectives
                 def evaluate_all_objectives(individual, costs, impact_matrix, traci_impact_cols):
                      adjusted_amounts = np.maximum(0, np.array(individual))
                      total_cost = np.dot(adjusted_amounts, costs)
                      total_impacts = []
                      for i in range(impact_matrix.shape[1]):
                 ...
                 all_objective_values = evaluate_all_objectives(best_individual_combined, costs, impact_matrix, traci_impact_cols)

                 st.metric("Total Cost", f"${all_objective_values[0]:.2f}")
                 # Find GWP for display
                 try:
                     gwp_idx = traci_impact_cols.index("kg CO2-Eq/Unit")
                     st.metric("Total GWP (kg CO2-Eq)", f"{all_objective_values[gwp_idx + 1]:.4f}")
                 except ValueError:
                     st.write("GWP data not available.")

                 st.subheader("Optimized Material Inputs")
                 optimized_inputs_df = pd.DataFrame({
                     'Material': materials,
                     'Base Amount': base_amounts,
                     'Optimized Amount': best_individual_combined
                 })
                 st.dataframe(optimized_inputs_df)

             else:
                  st.write("Optimization did not find a solution.")


        elif scenario == "Optimize Single Impact":
             if selected_impact:
                 st.write(f"Finding the optimal solution minimizing {selected_impact}.")

                 # Define a single objective evaluation function for the selected impact
                 def evaluate_single_impact(individual, impact_matrix, impact_col, traci_impact_cols):
                      adjusted_amounts = np.maximum(0, np.array(individual))
                      total_impact = 0.0
                      try:
                          impact_idx = traci_impact_cols.index(impact_col)
                          ...
                          st.warning(f"Impact column '{impact_col}' not found.")
                      return (total_impact,) # Return as tuple

                 best_individual_impact = run_single_objective_optimization(
                     evaluate_single_impact, POP_SIZE, NGEN, CXPB, MUTPB, low_bounds, high_bounds,
                     impact_matrix, selected_impact, traci_impact_cols # Pass additional arguments
                 )

                 st.subheader(f"Optimal Solution for Minimizing {selected_impact}")
                 if best_individual_impact:
                      # Evaluate this individual using the function that returns all objectives for display
                      def evaluate_all_objectives(individual, costs, impact_matrix, traci_impact_cols):
                           adjusted_amounts = np.maximum(0, np.array(individual))
                           total_cost = np.dot(adjusted_amounts, costs)
                      ...
                      all_objective_values = evaluate_all_objectives(best_individual_impact, costs, impact_matrix, traci_impact_cols)

                      st.metric("Total Cost", f"${all_objective_values[0]:.2f}")
                      st.metric(selected_impact, f"{best_individual_impact.fitness.values[0]:.4f}") # Display the minimized impact value
                      # Find GWP for display
                      try:
                          gwp_idx = traci_impact_cols.index("kg CO2-Eq/Unit")
                          st.metric("Total GWP (kg CO2-Eq)", f"{all_objective_values[gwp_idx + 1]:.4f}")
                      ...
                      optimized_inputs_df = pd.DataFrame({
                          'Material': materials,
                          'Base Amount': base_amounts,
                          'Optimized Amount': best_individual_impact
                      })
                      st.dataframe(optimized_inputs_df)
                 ...
                 st.warning("Please select a TRACI Impact Category to optimize.")

        elif scenario == "Optimize Cost Only":
             st.write("Finding the optimal solution minimizing Total Cost.")

             # Define a single objective evaluation function for cost only
             def evaluate_cost_only(individual, costs):
                  adjusted_amounts = np.maximum(0, np.array(individual))
                  total_cost = np.dot(adjusted_amounts, costs)
                  return (total_cost,) # Return as tuple

             best_individual_cost = run_single_objective_optimization(
                 evaluate_cost_only, POP_SIZE, NGEN, CXPB, MUTPB, low_bounds, high_bounds, costs # Pass costs
             )

             st.subheader("Optimal Solution for Minimizing Total Cost")
             if best_individual_cost:
                  # Evaluate this individual using the function that returns all objectives for display
                  def evaluate_all_objectives(individual, costs, impact_matrix, traci_impact_cols):
                       adjusted_amounts = np.maximum(0, np.array(individual))
                       total_cost = np.dot(adjusted_amounts, costs)
                  ...
                  all_objective_values = evaluate_all_objectives(best_individual_cost, costs, impact_matrix, traci_impact_cols)

                  st.metric("Total Cost", f"${best_individual_cost.fitness.values[0]:.2f}") # Display the minimized cost value
                  # Find GWP for display
                  try:
                      gwp_idx = traci_impact_cols.index("kg CO2-Eq/Unit")
                      st.metric("Total GWP (kg CO2-Eq)", f"{all_objective_values[gwp_idx + 1]:.4f}")
                  ...
                  optimized_inputs_df = pd.DataFrame({
                      'Material': materials,
                      'Base Amount': base_amounts,
                      'Optimized Amount': best_individual_cost
                  })
                  st.dataframe(optimized_inputs_df)
             else:
                 st.write("Optimization did not find a solution.")

# Save the content of cell 23f31b9b to a Python file
with open('streamlit_app.py', 'w') as f:
    f.write("""
import streamlit as st
import pandas as pd
